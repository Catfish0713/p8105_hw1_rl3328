...
```
The temperature in LGA, JFK and EWR seems to increase as time(hour) pass by in January 2013.
...
```
The mean for variable sample is `mean mean(pull(df,sample))`.
The mean for variable sample is `mean mean(pull(df,logic))`.
